...
Cypress.Commands.add('cleanQuizzes', () => {
...
Cypress.Commands.add('cleanQuizzesItemCombination', () => {
  dbCommand(` DELETE FROM item_answer_combinations;
...
Cypress.Commands.add('cleanItemCombinationQuestionsByName', questionName => {
  dbCommand(`WITH toDelete AS (SELECT qt.id as question_id FROM questions qt JOIN question_details qd ON qd.question_id = qt.id and qd.question_type='item_combination' where title like '%${questionName}%')
                  , comb AS (DELETE FROM item_combinations WHERE item_combinations.item_id IN ((SELECT items.id FROM items WHERE question_details_id IN (SELECT qd.id FROM toDelete JOIN question_details qd on qd.question_id = toDelete.question_id))))
                  , opt AS (DELETE FROM items WHERE question_details_id IN (SELECT qd.id FROM toDelete JOIN question_details qd on qd.question_id = toDelete.question_id))
                  , det AS (DELETE FROM question_details WHERE question_id in (SELECT question_id FROM toDelete))
                DELETE FROM questions WHERE id IN (SELECT question_id FROM toDelete);`);
});
