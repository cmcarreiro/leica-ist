...
export default class EditQuestionDialog extends Vue {
  ...
  async saveQuestion() {
    ...
    if (this.editQuestion.questionDetailsDto.type === 'multiple_choice')
      if (!this.editQuestion.questionDetailsDto.orderMatters)
        this.editQuestion.questionDetailsDto.listOfOptions.forEach(
          (option: any) => (option.relevance = 0)
        );
