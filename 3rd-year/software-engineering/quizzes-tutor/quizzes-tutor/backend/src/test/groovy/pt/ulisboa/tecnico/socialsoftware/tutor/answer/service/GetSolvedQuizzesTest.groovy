...
class GetSolvedQuizzesTest extends SpockTest {
    ...
    def "returns solved quiz with: quizType=#quizType | conclusionDate=#conclusionDate | resultsDate=#resultsDate"() {
        ...
        def answerDetails = new MultipleChoiceAnswer(questionAnswer, listOfOptions);
        ...
        ArrayList<Option> listOfOptionsIds = new ArrayList<>()
        listOfOptionsIds.add(option.getId())
        answer.getAnswerDetails().getListOfOptionIds().equals(listOfOptionsIds)
        solvedQuizDto.getCorrectAnswers().size() == 1
        def correct = solvedQuizDto.getCorrectAnswers().get(0)
        correct.getSequence() == 0
        correct.getCorrectAnswerDetails().getListOfCorrectOptionIds()[0] == option.getId()
