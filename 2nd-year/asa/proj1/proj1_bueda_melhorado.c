...
int visitaDFS(grafo* g, int n_pai) {
    ...
    while(v_filho != NULL) {
        if(!g->visitado[v_filho->valor]) {
            if(v_filho->valor < n_pai) {
                max_filhos = max(max_filhos, g->valores[v_filho->valor]);       /*ja sabemos a nota final*/
            }
            else {
                max_filhos = max(max_filhos, visitaDFS(g, v_filho->valor));     /*ainda nao sabemos a nota final*/
            }
        }
